...
Restart is memoized into `restartPromise` and `loadWorld`/`runQuery` await it first, so callers never race a half-restarted engine; the reload inside `restart` calls `doLoadWorld` directly (calling `loadWorld` there would await `restartPromise` and deadlock). Row extraction is positional via `getChildAt` so duplicate/aliased column names don't collapse.
