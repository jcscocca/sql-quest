...
- **Content review notes (2026-07-19)**: the `distinct` bank (d-1/d-4/d-6) deliberately uses basic ascending `ORDER BY`, which its own lesson example pre-teaches, even though `order-limit` is a curriculum sibling rather than a prerequisite — accepted scope choice, not an oversight. The hint-3 ≡ reference equivalence guarantee is verified at review time with the real comparator; the harness itself only EXPLAIN-parses hint SQL.
