...
- **Collection mechanic (capped dynamic + authored, finalized 2026-07-19; random cap retuned 3→1 on 2026-07-22)**: on each exercise's FIRST correct solve, 1 random *new* Pokémon appearing in the user's actual result rows is caught (values matched against the world's name list), plus any authored `collectibles` bonuses. The random cap was lowered from 3 so authored signature catches stand out instead of arriving in a pile of filler. Re-solves and review solves don't catch — collection growth comes from progression and new content. Node completion awards a skill badge; completing a region awards a region badge.
- **Collection page**: reached via the header collection count — grid of caught Pokémon as type-colored text tiles plus the badge shelf. No sprites (remote images would break offline; bundled sprites are a possible Stage 3 flourish) — bundled sprites shipped 2026-07-19 (collection-sprites project), offline preserved by committing the assets.
