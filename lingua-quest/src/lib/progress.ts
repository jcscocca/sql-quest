// The shared backbone: XP, streak, per-skill mastery + review schedule, the
// vocabulary collection, and badges. Persisted to IndexedDB — no accounts.
...
import type { Vocab } from './content'
...
interface ProgressStore extends ProgressState {
  ...
  addVocab(course: string, entries: Vocab[]): VocabEntry[]
...
const KEY = 'lingua-quest-progress'
...
/** Backfill any completed-but-unscheduled skill so it enters review. */
...
export const useProgress = create<ProgressStore>((set, get) => ({
  ...
  addVocab(course, entries) {
    if (entries.length === 0) return []
    const s = get()
    const fresh = entries.filter(
      e => !s.vocab.some(v => v.course === course && v.es === e.es),
    )
    if (fresh.length === 0) return []
    const tagged: VocabEntry[] = fresh.map(e => ({ course, es: e.es, en: e.en, pos: e.pos }))
    const next: ProgressState = { ...dataOf(s), vocab: [...s.vocab, ...tagged] }
    set(next)
    persist(next)
    return tagged
  },
