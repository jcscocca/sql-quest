// Spaced repetition. When a skill is completed it enters a review rotation on
// an expanding schedule (2 → 4 → 8 … days). Getting a review right pushes the
// next one further out; getting it wrong pulls mastery down and resets the
// interval. Daily Review resurfaces the skills that are most overdue.

import { addDays, dayDiff } from './xp'
import type { Exercise, SkillBank } from './content'
...
/** Mastery as it should read today, decaying while a review is overdue. */
...
export function reviewOutcome(
  ...
  success: boolean,
  today: string,
): Required<Omit<ReviewableSkill, 'completed'>> {
  if (success) {
...
/** Build today's review queue: most-overdue skills first, capped per skill. */
